...
def get_default_args():
    ...
    parser.add_argument("--cross_attn_heads", type=int, default=1,
                        help="Number of attention heads for cross-modal attention (default: 3, must divide 42 and 24)")
    
    # Hand Graph Convolutional Network settings
    parser.add_argument("--use_hand_gcn", type=bool, default=True,
                        help="Determines whether to use Hand Graph Convolutional Network for spatial joint modeling")
...
def train(args):
    ...
    if args.FIM:
        slr_model = SiFormer(num_classes=args.num_classes, num_hid=args.num_seq_elements, attn_type=args.attn_type,
                              ...
                              cross_attn_heads=args.cross_attn_heads, use_hand_gcn=args.use_hand_gcn)
    else:

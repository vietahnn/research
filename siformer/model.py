...
class FeatureIsolatedTransformer(nn.Transformer):
    ...
    def forward(self, src: list, tgt: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        # Store encoder outputs for contrastive learning (before cross-attention)
        self.l_hand_memory = l_hand_memory
        self.r_hand_memory = r_hand_memory
        ...
        if return_features:
            return output, self.l_hand_memory, self.r_hand_memory, self.body_memory
        return output
...
class SiFormer(nn.Module):
    ...
    def forward(self, l_hand, r_hand, body, training, return_features=False):
        ...
        if return_features:
            transformer_output, lh_feat, rh_feat, body_feat = self.transformer(
        ...
        if return_features:
            return out, lh_feat, rh_feat, body_feat
        return out
